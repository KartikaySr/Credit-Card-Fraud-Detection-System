  ...
  return (
    <div className="grids-card p-6 h-[400px] flex flex-col relative overflow-hidden group">
      <div className="flex justify-between items-start z-10">
        <div>
          <h3 className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-amber-400 via-gold to-yellow-200">
            Transaction Velocity
          </h3>
          <p className="text-sm text-gray-400">TPS (Transactions Per Second)</p>
        </div>
        <div className="text-3xl font-mono text-gold animate-pulse">{current}</div>
      </div>
      ...
      <div className="flex-1 flex items-end gap-1 mt-6 z-10">
        {history.map((val, i) => {
          ...
          return (
            <div 
              key={i} 
              className="flex-1 bg-gradient-to-t from-emerald-600/50 to-gold/80 rounded-t hover:opacity-80 transition-opacity"
