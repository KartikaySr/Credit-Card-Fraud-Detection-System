  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        if (eventCount > 0) {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/intelligence/biometrics`, {
            method: 'POST',
          });
    ...
    <div className="space-y-6 animate-fade-in pb-20">
      ...
      <header className="flex justify-between items-end border-b border-white/10 pb-6">
        ...
        <button 
          ...
          className={`px-6 py-2 rounded-full font-bold transition-all shadow-lg ${isTracking ? 'bg-red-500 hover:bg-red-600 text-white shadow-red-500/20' : 'bg-gold hover:bg-gold-light text-black shadow-[0_0_15px_rgba(230,197,82,0.4)]'}`}
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div 
            ref={containerRef}
            className="grids-card h-[400px] rounded-2xl border border-white/10 relative overflow-hidden flex flex-col items-center justify-center cursor-crosshair group bg-[#0a0a0a]"
          >
            {/* Grid Background */}
            <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.03)_1px,transparent_1px)] bg-[size:40px_40px]"></div>
            
            {!isTracking ? (
              <div className="text-center z-10 p-8 grids-card rounded-2xl border border-white/5 bg-black/60">
              ...
              <div className="text-center z-10 pointer-events-none">
                <div className="w-48 h-48 rounded-full border border-gold/30 flex items-center justify-center animate-[spin_4s_linear_infinite]">
                  <div className="w-32 h-32 rounded-full border border-emerald-500/30 animate-[spin_3s_linear_infinite_reverse]"></div>
                </div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="font-mono text-gold font-bold bg-black/50 px-3 py-1 rounded">
                    CAPTURING VECTORS
                  </span>
                </div>
              </div>
          ...
          <div className="grids-card h-[200px] rounded-2xl border border-white/10 p-6 flex flex-col">
        ...
        {/* Live Analysis Panel */}
        <div className="grids-card rounded-2xl border border-white/10 p-6 flex flex-col bg-black/40">
          <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
            <Fingerprint className="text-gold w-6 h-6" />
